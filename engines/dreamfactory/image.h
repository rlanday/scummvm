...
#ifndef DREAMFACTORY_IMAGE_H
#define DREAMFACTORY_IMAGE_H

#include "common/scummsys.h"
#include "common/array.h"

#include "dreamfactory/palette.h"
...
/**
 * A decoded DreamFactory "cel": an 8-bit palettised image with per-pixel
...
struct CelImage {
	...
	bool isOpaque(int x, int y) const {
		if (x < 0 || y < 0 || x >= width || y >= height)
			return false;
		const uint offset = static_cast<uint>(y) * width + x;
		return offset < opaque.size() && opaque[offset] != 0;
	}
};

/**
 * DREAMFACTORY MOV VIDEO FILE FORMAT (.MOV)
 * =====================================
 * Despite the extension these are NOT QuickTime; a .MOV is a DreamFactory
 * DreamFactory LPPALPPA container (see Archive) whose resources hold the
 ...
 *
 * These are produced by the DreamFactory full-screen frame decompressor (TI.EXE
...
/**
 * A persistent full-screen framebuffer that decodes a DreamFactory movie one
 * frame at a time.
 *
...
/**
 * Locate and parse the embedded Macintosh 'clut' palette in a DreamFactory
