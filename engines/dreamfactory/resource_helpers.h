...
#ifndef DREAMFACTORY_RESOURCE_HELPERS_H
#define DREAMFACTORY_RESOURCE_HELPERS_H
...
public:
	class Iterator {
	public:
		Iterator(const RecordRange &range, uint32 index) : _range(range), _index(index) {}
		ResourceView operator*() const { return _range.record(_index); }
		Iterator &operator++() { ++_index; return *this; }
		bool operator!=(const Iterator &other) const {
			return &_range != &other._range || _index != other._index;
		}
	private:
		const RecordRange &_range;
		uint32 _index;
	};
...
/** Owns a container's bytes and the archive whose stream borrows those bytes. */
class ResourceFile {
public:
	ResourceFile() {}
	bool open(const Common::String &name, const char *kind);
	void close() {
		_archive.close();
		_data.clear();
	}
...
private:
	...
	// Reverse destruction order keeps the backing bytes alive until the
	// archive has destroyed its borrowing stream. Views expire on close/open.
	Common::Array<byte> _data;
	Archive _archive;
};

inline int resourceIndexById(const Archive &archive, uint32 id) {
	return archive.resourceIndexById(id);
}
