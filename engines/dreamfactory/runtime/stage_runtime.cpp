...
bool StageRuntime::dispatchFlatMessage(DreamFactoryEngine &engine, const Common::SharedPtr<Stage> &dispatchStage,
	...
	if (dispatchNode < 0 || static_cast<uint32>(dispatchNode) >= dispatchStage->nodeCount()) {
		warning("DreamFactory: stage '%s' has no flat index %d",
...
bool StageRuntime::queueStageNodeRedraw(DreamFactoryEngine &engine, int targetNode) {
	if (!stage() || !stage()->isOpen()) {
		warning("DreamFactory: gotoflat(%d) with no stage open", targetNode + 1);
		return false;
	}
...
void StageRuntime::openStageFile(DreamFactoryEngine &engine, const Common::String &name) {
	if (name.empty())
		return;
	debug(1, "DreamFactory: openstagefile('%s') from stage '%s' flat '%s'",
			name.c_str(), currentStage().c_str(), currentFlat().c_str());
	Common::SharedPtr<Stage> newStage(new Stage());
	if (!newStage->open(name)) {
		debug(1, "DreamFactory: openstagefile('%s') failed", name.c_str());
		return;
	}
	clearShellFrame();
	clearNodeFrame();
	stage() = newStage;
	visible() = true;
	debug(1, "DreamFactory: stage '%s' open (%u nodes)", name.c_str(), stage()->nodeCount());
...
void StageRuntime::closeStageFile(DreamFactoryEngine &engine) {
	if (!stage() || !stage()->isOpen())
		return;
	debug(1, "DreamFactory: closestagefile() closing stage '%s' flat '%s'",
...
void StageRuntime::gotoFlat(DreamFactoryEngine &engine, const Value &flat) {
	if (!stage() || !stage()->isOpen())
		return;
	debug(1, "DreamFactory: gotoflat(%s) in stage '%s' flat '%s'",
	...
	if (targetNode < 0 || static_cast<uint32>(targetNode) >= stage()->nodeCount()) {
		warning("DreamFactory: gotoflat('%s') not found in stage '%s'",
				flat.toString().c_str(), stage()->name().c_str());
		return;
	}
	if (targetNode == node())
		return;
	debug(1, "DreamFactory: gotoflat(%s) resolved node %d", flat.toString().c_str(), targetNode);
...
void StageRuntime::restoreSnapshot(const Snapshot &snapshot, const Common::SharedPtr<Stage> &preparedStage) {
	reset();
	stage() = preparedStage;
	if (!stage())
		return;
...
void StageRuntime::sendToStage(DreamFactoryEngine &engine, const Common::String &message, const Common::Array<Value> &args) {
	if (!stage() || !stage()->isOpen()) {
		warning("DreamFactory: sendtostage('%s') with no stage open", message.c_str());
		return;
	}
	Common::SharedPtr<Stage> dispatchStage = stage();
...
Value StageRuntime::sendToStageFx(DreamFactoryEngine &engine, const Common::String &message, const Common::Array<Value> &args) {
	if (!stage() || !stage()->isOpen()) {
		warning("DreamFactory: sendtostagefx('%s') with no stage open", message.c_str());
		return Value();
	}
	Common::SharedPtr<Stage> dispatchStage = stage();
...
void StageRuntime::sendToFlat(DreamFactoryEngine &engine, const Common::String &flat, const Common::String &message,
		const Common::Array<Value> &args) {
	if (!stage() || !stage()->isOpen()) {
		warning("DreamFactory: sendtoflat('%s') with no stage open", flat.c_str());
		return;
	}
	Common::SharedPtr<Stage> dispatchStage = stage();
	int dispatchNode = flat.empty() ? node() : dispatchStage->findNode(flat);
	if (dispatchNode < 0 || static_cast<uint32>(dispatchNode) >= dispatchStage->nodeCount()) {
		warning("DreamFactory: stage '%s' has no flat named '%s'",
...
Value StageRuntime::sendToFlatFx(DreamFactoryEngine &engine, const Common::String &flat, const Common::String &message,
		const Common::Array<Value> &args) {
	if (!stage() || !stage()->isOpen()) {
		warning("DreamFactory: sendtoflatfx('%s') with no stage open", flat.c_str());
		return Value();
	}
	Common::SharedPtr<Stage> dispatchStage = stage();
	int dispatchNode = flat.empty() ? node() : dispatchStage->findNode(flat);
...
void StageRuntime::sendToButton(DreamFactoryEngine &engine, const Common::String &flat, const Common::String &button,
		const Common::String &message, const Common::Array<Value> &args) {
	if (!stage() || !stage()->isOpen()) {
		warning("DreamFactory: sendtobutton('%s') with no stage open", button.c_str());
		return;
	}
	Common::SharedPtr<Stage> dispatchStage = stage();
	int dispatchNode = flat.empty() ? node() : dispatchStage->findNode(flat);
	...
	if (!dispatchStage->hasButton(static_cast<uint32>(dispatchNode), button)) {
		warning("DreamFactory: stage '%s' flat '%s' has no button named '%s'",
	...
	debug(1, "DreamFactory: sendtobutton('%s', '%s') -> %s(%u args)",
...
Value StageRuntime::sendToButtonFx(DreamFactoryEngine &engine, const Common::String &flat, const Common::String &button,
		const Common::String &message, const Common::Array<Value> &args) {
	if (!stage() || !stage()->isOpen()) {
		warning("DreamFactory: sendtobuttonfx('%s') with no stage open", button.c_str());
		return Value();
	}
	Common::SharedPtr<Stage> dispatchStage = stage();
	int dispatchNode = flat.empty() ? node() : dispatchStage->findNode(flat);
...
void StageRuntime::renderStageNode(DreamFactoryEngine &engine, int targetNode, bool resetCursor,
		bool present) {
	if (!stage() || !stage()->isOpen()) {
		warning("DreamFactory: sendtostage(%d) with no stage open", targetNode);
		return;
	}
	node() = targetNode;
	...
	debug(1, "DreamFactory: rendered stage '%s' node %d (%ux%u)",
			stage()->name().c_str(), targetNode, frame.width, frame.height);
}

void StageRuntime::repaintDirtyStageRects(DreamFactoryEngine &engine, bool present) {
	if (!stage() || !stage()->isOpen())
		return;
...
bool StageRuntime::pointInButton(const Common::String &flat,
	...
	debug(1, "DreamFactory: pointinbutton('%s', '%s', %d,%d) -> %s",
