...
void ActorRuntime::dispatchWalkComplete(DreamFactoryEngine &engine, const Common::String &name) {
...
void ActorRuntime::dispatchTurnComplete(DreamFactoryEngine &engine, const Common::String &name) {
...
bool ActorRuntime::recoverExtraBaseActor(DreamFactoryEngine &engine, const Common::String &name) {
	...
	debug(1, "DreamFactory: recovering extra base actor '%s' by opening extra.cst",
			name.c_str());
	return true;
}

bool ActorRuntime::recoverGeneratedExtraActor(DreamFactoryEngine &engine, const Common::String &name) {
	...
	debug(1, "DreamFactory: recovering generated extra actor '%s' via %s.setupactor('%s')",
			name.c_str(), sourceName.c_str(), where.c_str());
	sendToActor(engine, sourceName, "setupactor", args);
	return findActorRef(name).actor;
}

bool ActorRuntime::resolveActorStar(DreamFactoryEngine &engine, Cast::Actor &actor) {
...
void ActorRuntime::refreshActorStarPositions(DreamFactoryEngine &engine) {
	if (!engine._setRuntime.set() || !engine._setRuntime.set()->isOpen())
		return;
...
void ActorRuntime::collectWorldActors(DreamFactoryEngine &engine, Common::Array<DrawEntry> &draw,
...
void ActorRuntime::openCastFile(DreamFactoryEngine &engine, const Common::String &name) {
	Common::String key = name;
	key.toLowercase();
	if (findCastShared(key)) {
		debug(1, "DreamFactory: cast '%s' already open", key.c_str());
		return;
	}
...
void ActorRuntime::closeCastFile(DreamFactoryEngine &engine, const Common::String &name) {
	Common::String key = name;
	key.toLowercase();
	for (uint i = 0; i < _casts.size(); ++i) {
		if (_casts[i]->name() == key) {
			...
			debug(1, "DreamFactory: cast '%s' closed", key.c_str());
			_casts.remove_at(i);
			engine._propRuntime.setDirty(true);
			return;
		}
	}
	debug(1, "DreamFactory: closecastfile('%s'): cast not open", key.c_str());
}

void ActorRuntime::actorInstance(DreamFactoryEngine &engine, const Common::String &source, const Common::String &newName) {
	ActorRef ref = findActorRef(source);
	if (!ref.actor || !ref.cast) {
		warning("DreamFactory: actorinstance('%s', '%s'): source actor not found",
				source.c_str(), newName.c_str());
		return;
	}
	if (newName.size() > kMaxInstanceNameLength) {
		warning("DreamFactory: actorinstance('%s', '%s'): name too long",
				source.c_str(), newName.c_str());
		return;
	}
	...
	if (ref.cast->addActorInstance(*ref.actor, newName)) {
		debug(1, "DreamFactory: actorinstance('%s', '%s')", source.c_str(), newName.c_str());
		refreshActorStarPositions(engine);
		engine._propRuntime.setDirty(true);
	}
}

void ActorRuntime::sendToCast(DreamFactoryEngine &engine, const Common::String &castName, const Common::String &message,
		const Common::Array<Value> &args) {
	debug(1, "DreamFactory: sendtocast('%s') -> %s(%u args)", castName.c_str(),
			message.c_str(), args.size());
	Common::SharedPtr<Cast> cast = findCastShared(castName);
	if (!cast) {
		warning("DreamFactory: sendtocast('%s'): cast not open", castName.c_str());
		return;
	}
...
Value ActorRuntime::sendToCastFx(DreamFactoryEngine &engine, const Common::String &castName, const Common::String &message,
		const Common::Array<Value> &args) {
	debug(1, "DreamFactory: sendtocastfx('%s') -> %s(%u args)", castName.c_str(),
			message.c_str(), args.size());
	Common::SharedPtr<Cast> cast = findCastShared(castName);
	if (!cast) {
		warning("DreamFactory: sendtocastfx('%s'): cast not open", castName.c_str());
		return Value();
	}
...
void ActorRuntime::sendToActor(DreamFactoryEngine &engine, const Common::String &actorName, const Common::String &message,
		const Common::Array<Value> &args) {
	debug(1, "DreamFactory: sendtoactor('%s') -> %s(%u args)", actorName.c_str(),
			message.c_str(), args.size());
	...
	if (!ref.actor) {
		warning("DreamFactory: sendtoactor('%s'): no such actor", actorName.c_str());
		return;
	}
...
Value ActorRuntime::sendToActorFx(DreamFactoryEngine &engine, const Common::String &actorName, const Common::String &message,
		const Common::Array<Value> &args) {
	debug(1, "DreamFactory: sendtoactorfx('%s') -> %s(%u args)", actorName.c_str(),
			message.c_str(), args.size());
	ActorRef ref = findActorRef(actorName);
	if (!ref.actor) {
		if (recoverExtraBaseActor(engine, actorName) || recoverGeneratedExtraActor(engine, actorName))
	...
	if (!ref.actor) {
		warning("DreamFactory: sendtoactorfx('%s'): no such actor", actorName.c_str());
		return Value();
	}
...
bool ActorRuntime::getActorVisible(const Common::String &name) const {
	...
	if (!ref.actor) {
		warning("DreamFactory: actorvisible('%s'): no such actor", name.c_str());
		return false;
	}
	return ref.actor->visible;
}

bool ActorRuntime::setActorVisible(DreamFactoryEngine &engine, const Common::String &name, bool visible) {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actorvisible('%s'): no such actor", name.c_str());
		return false;
...
Common::String ActorRuntime::getActorSet(const Common::String &name) const {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actorset('%s'): no such actor", name.c_str());
		return Common::String();
	}
	return ref.actor->setName;
}

Common::String ActorRuntime::setActorSet(DreamFactoryEngine &engine, const Common::String &name, const Common::String &newSet) {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actorset('%s'): no such actor", name.c_str());
		return Common::String();
...
Common::String ActorRuntime::getActorStar(const Common::String &name) const {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actorstar('%s'): no such actor", name.c_str());
		return Common::String();
	}
	return ref.actor->sceneName;
}

Common::String ActorRuntime::setActorStar(DreamFactoryEngine &engine, const Common::String &name, const Common::String &newStar) {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actorstar('%s'): no such actor", name.c_str());
		return Common::String();
...
Common::String ActorRuntime::getActorPose(const Common::String &name) const {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actorpose('%s'): no such actor", name.c_str());
		return Common::String();
	}
	return ref.actor->shapeName;
}

Common::String ActorRuntime::setActorPose(DreamFactoryEngine &engine, const Common::String &name, const Common::String &newPose) {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actorpose('%s'): no such actor", name.c_str());
		return Common::String();
...
void ActorRuntime::actorXYZ(DreamFactoryEngine &engine, const Common::String &name, int x, int y, int z) {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actorxyz('%s'): no such actor", name.c_str());
		return;
	}
...
int ActorRuntime::actorXYZ(DreamFactoryEngine &engine, const Common::String &name, int selector) const {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actorxyz('%s'): no such actor", name.c_str());
		return 0;
...
int ActorRuntime::getActorDeg(const Common::String &name) const {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actordeg('%s'): no such actor", name.c_str());
		return 0;
	}
	return ref.actor->angle;
}

int ActorRuntime::setActorDeg(DreamFactoryEngine &engine, const Common::String &name, int newDeg) {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actordeg('%s'): no such actor", name.c_str());
		return 0;
...
int ActorRuntime::getActorDist(DreamFactoryEngine &engine, const Common::String &name) const {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actordist('%s'): no such actor", name.c_str());
		return 0;
	}
...
void ActorRuntime::setActorDist(DreamFactoryEngine &engine, const Common::String &name, int newDist) {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actordist('%s'): no such actor", name.c_str());
		return;
...
int ActorRuntime::getActorValue(const Common::String &name) const {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actorvalue('%s'): no such actor", name.c_str());
		return 0;
	}
...
Common::String ActorRuntime::getActorOwner(const Common::String &name) const {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actorowner('%s'): no such actor", name.c_str());
		return Common::String();
	}
...
void ActorRuntime::actorZClip(DreamFactoryEngine &engine, const Common::String &name, int zClip) {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actorzclip('%s'): no such actor", name.c_str());
		return;
	}
...
void ActorRuntime::actorSpeed(const Common::String &name, int speed) {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actorspeed('%s'): no such actor", name.c_str());
		return;
	}
	ref.actor->speed = speed;
}

void ActorRuntime::actorScale(DreamFactoryEngine &engine, const Common::String &name, int scale) {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actorscale('%s'): no such actor", name.c_str());
		return;
	}
...
void ActorRuntime::actorTurn(const Common::String &name, int turn) {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: actorturn('%s'): no such actor", name.c_str());
		return;
	}
	ref.actor->turn = turn;
}

void ActorRuntime::turnToDeg(DreamFactoryEngine &engine, const Common::String &name, int deg) {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: turntodeg('%s'): no such actor", name.c_str());
		return;
	}
...
void ActorRuntime::queueAnimatedWalk(DreamFactoryEngine &engine, Cast::Actor &actor,
...
void ActorRuntime::walkToStar(DreamFactoryEngine &engine, const Common::String &name, const Common::String &star) {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: walktostar('%s'): no such actor", name.c_str());
		return;
	}
	...
	if (engine._setRuntime.set() && engine._setRuntime.set()->isOpen() &&
		...
		debug(1, "DreamFactory: walktostar('%s', '%s') queued dist %d", name.c_str(), star.c_str(),
				walkDistance(ref.actor->x - sx, ref.actor->y - sy, ref.actor->z - sz));
		queueAnimatedWalk(engine, *ref.actor, name, star, sx, sy, sz);
		return;
	}
	debug(1, "DreamFactory: walktostar('%s', '%s') resolved immediately (actor off-set)",
...
void ActorRuntime::walkOnPath(DreamFactoryEngine &engine, const Common::String &name,
		const Common::String &, const Common::String &dest) {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: walkonpath('%s'): no such actor", name.c_str());
		return;
	}
	...
	if (engine._setRuntime.set() && engine._setRuntime.set()->isOpen() &&
		...
		debug(1, "DreamFactory: walkonpath('%s' -> '%s') queued as direct walk", name.c_str(), dest.c_str());
		queueAnimatedWalk(engine, *ref.actor, name, dest, sx, sy, sz);
		return;
	}
	debug(1, "DreamFactory: walkonpath('%s' -> '%s') resolved immediately (actor off-set)",
			name.c_str(), dest.c_str());
	setActorStar(engine, name, dest);
	dispatchWalkComplete(engine, name);
}

void ActorRuntime::walkToXYZ(DreamFactoryEngine &engine, const Common::String &name, int x, int y, int z) {
	ActorRef ref = findActorRef(name);
	if (!ref.actor) {
		warning("DreamFactory: walktoxyz('%s'): no such actor", name.c_str());
		return;
	}
	...
	if (engine._setRuntime.set() && engine._setRuntime.set()->isOpen() &&
			ref.actor->setName.equalsIgnoreCase(engine._setRuntime.set()->setName())) {
		debug(1, "DreamFactory: walktoxyz('%s', %d, %d, %d) queued", name.c_str(), x, y, z);
		queueAnimatedWalk(engine, *ref.actor, name, Common::String(),
				static_cast<int16>(x), static_cast<int16>(y), static_cast<int16>(z));
		return;
	}
	debug(1, "DreamFactory: walktoxyz('%s', %d, %d, %d) resolved immediately (actor off-set)",
			name.c_str(), x, y, z);
...
void ActorRuntime::advanceWalks(DreamFactoryEngine &engine) {
	if (_walks.empty())
		return;
...
int ActorRuntime::starXYZ(DreamFactoryEngine &engine, const Common::String &name, int selector) const {
	if (!engine._setRuntime.set() || !engine._setRuntime.set()->isOpen())
		return 0;
