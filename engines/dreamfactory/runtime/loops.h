...
#ifndef DREAMFACTORY_RUNTIME_LOOPS_H
#define DREAMFACTORY_RUNTIME_LOOPS_H
...
public:
	...
	/**
	 * Named SFX registration persisted in saves. Unlike ScheduledLoop, this
	 * currently has no processing pass: makeCricket() requests playback once,
	 * and pause/stop change registration state without controlling the mixer.
	 */
	struct CricketState {
		Common::String name;
		bool paused = false;
	...
	void makeCricket(DreamFactoryEngine &engine, const Common::String &name);
	void stopCricket(const Common::String &name);
	void pauseCricket(const Common::String &kind, bool paused);
	void processScheduledLoops(DreamFactoryEngine &engine);
