...
// reversed again.
void DreamFactoryEngine::dispatchWithScopes(const Script *scope1, const Script *scope2,
...
Value DreamFactoryEngine::dispatchWithScopesValue(const Script *scope1, const Script *scope2,
		const Common::String &self, const Common::String &targetProp,
		const Common::String &message, const Common::Array<Value> &args,
		const char *debugContext) {
...
Value DreamFactoryEngine::dispatchWithThreeScopesValue(const Script *scope1, const Script *scope2,
	...
	if (!handled)
		debug(1, "DreamFactory: %s message '%s' unhandled", debugContext, message.c_str());
...
Value DreamFactoryEngine::dispatchWithScopeChainValue(const Common::Array<const Script *> &scopes,
		const Common::String &self, const Common::String &targetProp,
		const Common::String &message, const Common::Array<Value> &args,
		const char *debugContext) {
...
Value DreamFactoryEngine::dispatchWithScopeChainContextsValue(const Common::Array<ScriptVM::LibraryScope> &scopes,
		const Common::String &self, const Common::String &targetProp,
		const Common::String &message, const Common::Array<Value> &args,
		const char *debugContext) {
	Common::String prevSelf = _vm.contextSelf();
...
void DreamFactoryEngine::dispatchSetMessage(const Common::String &message, const Common::Array<Value> &args) {
...
Value DreamFactoryEngine::dispatchSetMessageValue(const Common::String &message, const Common::Array<Value> &args) {
	if (!_setRuntime.set() || !_setRuntime.set()->isOpen() || message.empty())
		return Value();
	Common::SharedPtr<Script> setScript = _setRuntime.set()->setScriptShared();
...
Value DreamFactoryEngine::sendToSetFx(const Common::String &message, const Common::Array<Value> &args) {
	return dispatchSetMessageValue(message, args);
}

void DreamFactoryEngine::sendToSet(const Common::String &message, const Common::Array<Value> &args) {
	dispatchSetMessage(message, args);
}

void DreamFactoryEngine::dispatchSceneMessage(uint32 scene, const Common::String &message,
...
bool DreamFactoryEngine::closeCurrentSceneForNavigation() {
