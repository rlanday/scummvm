...
// from the player's DreamFactory executable rather than shipped with ScummVM. The
...
Common::PEResources *CursorRuntime::gameExe() {
	...
	warning("DreamFactory: could not locate %s (or any executable holding the "
...
bool CursorRuntime::tryLoadExe(const Common::FSNode &node, bool requireCursors) {
	...
	if (requireCursors) {
		...
		// only the DreamFactory runtime carries the cursor group.
	...
	debug(1, "DreamFactory: cursor resources loaded from '%s'", node.getPath().toString().c_str());
...
bool CursorRuntime::scanForExe(const Common::FSNode &dir, int depth, bool byNameOnly) {
	...
	if (!dir.getChildren(entries, Common::FSNode::kListAll))
		return false;
	for (const Common::FSNode &entry : entries) {
		if (entry.isDirectory())
			continue;
		Common::String name = entry.getName();
		name.toUppercase();
		if (byNameOnly ? (name != executableName) : !name.hasSuffix(".EXE"))
			continue;
		if (tryLoadExe(entry, !byNameOnly))
			return true;
	}
	if (depth <= 0)
		return false;
	for (const Common::FSNode &entry : entries) {
		if (entry.isDirectory() && scanForExe(entry, depth - 1, byNameOnly))
			return true;
	}
	return false;
}
...
bool CursorRuntime::setCursor(const Common::String &name) {
	...
	if (!group || group->cursors.empty()) {
		debug(1, "DreamFactory: cursor '%s' missing/empty in %s", name.c_str(),
	...
	_activeCursor = name;
	_appliedCursor = name;
	debug(1, "DreamFactory: cursor -> %s", name.c_str());
	return true;
}

bool DreamFactoryEngine::setGameCursor(const Common::String &name) {
