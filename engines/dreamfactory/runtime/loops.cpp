...
void LoopRuntime::makeLoop(const Common::String &kind, const Common::String &target,
	...
	if (kind.empty() || message.empty()) {
		warning("DreamFactory: makeloop('%s', '%s', '%s', %d): invalid arguments",
	...
	debug(2, "DreamFactory: makeloop('%s', '%s', '%s', %d)",
			kind.c_str(), target.c_str(), message.c_str(), delay);
}
...
void LoopRuntime::makeCricket(DreamFactoryEngine &engine, const Common::String &name) {
	if (name.empty())
		return;
	for (CricketState &cricket : _crickets) {
		if (cricket.name.equalsIgnoreCase(name)) {
...
void LoopRuntime::stopCricket(const Common::String &name) {
	...
	debug(2, "DreamFactory: stopcricket('%s')", name.c_str());
}
...
void LoopRuntime::pauseCricket(const Common::String &kind, bool paused) {
	if (kind.equalsIgnoreCase("all")) {
		...
		for (CricketState &cricket : _crickets)
			cricket.paused = paused;
	} else {
		for (CricketState &cricket : _crickets)
			if (cricket.name.equalsIgnoreCase(kind))
				cricket.paused = paused;
	}
	debug(2, "DreamFactory: pausecricket('%s', %d)", kind.c_str(), paused ? 1 : 0);
}

void LoopRuntime::processScheduledLoops(DreamFactoryEngine &engine) {
	...
	for (uint32 i = 0; i < _scheduledLoops.size();) {
		...
		case ScheduledLoop::kActor:
			...
			break;
		default:
			debug(1, "DreamFactory: makeloop kind '%s' unhandled", loop.kind.c_str());
		}
