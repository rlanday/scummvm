...
#ifndef DREAMFACTORY_RUNTIME_PUPPET_RUNTIME_H
#define DREAMFACTORY_RUNTIME_PUPPET_RUNTIME_H
...
public:
	...
	void closePuppetFile(DreamFactoryEngine &engine);
	void sendToPuppet(DreamFactoryEngine &engine, const Common::String &puppetName,
			const Common::String &message, const Common::Array<Value> &args);
	Value sendToPuppetFx(DreamFactoryEngine &engine, const Common::String &puppetName,
			const Common::String &message, const Common::Array<Value> &args);
	void puppetScript(const Common::String &name);
	void puppetClear(DreamFactoryEngine &engine);
	void puppetSpeak(DreamFactoryEngine &engine, const Common::String &name, int mode);
	void puppetBevel(DreamFactoryEngine &engine, const Common::String &name, int mode);
	void puppetGrab(bool enabled);
	int puppetEvent(DreamFactoryEngine &engine, int timeout);
	Common::String getPuppetBase() const;
	Common::String setPuppetBase(const Common::String &newBase);
	bool getPuppetVisible() const;
	bool setPuppetVisible(DreamFactoryEngine &engine, bool visible);
	bool renderCurrentFrame(DreamFactoryEngine &engine, bool present);
...
private:
	...
	bool captureGrabBackdrop(DreamFactoryEngine &engine, Common::Array<byte> &backdrop);
	bool paintGrabBackdrop(DreamFactoryEngine &engine, Graphics::Surface &screen,
			const Common::Array<byte> *cachedBackdrop);
	bool renderFrame(DreamFactoryEngine &engine, const Puppet::ActionEntry &action,
			uint32 frameIndex, bool present,
			const Common::Array<byte> *cachedBackdrop = nullptr);
	void renderBevels(DreamFactoryEngine &engine, bool present);
	void drawBevelHighlight(DreamFactoryEngine &engine, const Common::Rect &rect);
	void strokeBevelHighlight(Graphics::Surface &screen, const Common::Rect &rect) const;
	void playAction(DreamFactoryEngine &engine, const Puppet::ActionEntry &action);
	 ...
	 */
	bool replayLastSpokenAction(DreamFactoryEngine &engine);
