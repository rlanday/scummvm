...
uint32 DreamFactoryEngine::handleMovieHotkeys(const Common::Event &event, bool skippable,
...
bool DreamFactoryEngine::handleGlobalKey(const Common::Event &event) {
	if (event.type != Common::EVENT_KEYDOWN)
		return false;
...
void DreamFactoryEngine::showAboutDialog() {
...
bool DreamFactoryEngine::questionDialog(const Common::String &message) {
...
void DreamFactoryEngine::requestQuit() {
	Engine::quitGame();
}
