...
bool DreamFactoryEngine::resolveClut(const Common::String &name, Palette &rgb) {
	...
	warning("DreamFactory: clut '%s' not resolvable yet", name.c_str());
	return false;
}
...
void DreamFactoryEngine::updatePaletteGammaTable() {
...
void DreamFactoryEngine::programPalette(const Palette &rgb) {
...
void DreamFactoryEngine::blackScreen() {
	...
	_system->updateScreen();
	debug(1, "DreamFactory: blackscreen()");
}

void DreamFactoryEngine::message(const Common::String &text) {
	...
	debug(1, "DreamFactory message: %s", text.c_str());
}
...
void DreamFactoryEngine::noteDialog(const Common::String &text) {
	debug(1, "DreamFactory notedialog: %s", text.c_str());
	GUI::MessageDialog dialog(text);
	dialog.runModal();
}

void DreamFactoryEngine::flushEvents() {
	_deferredInputEvents.clear();
	_eventMan->purgeMouseEvents();
	_eventMan->purgeKeyboardEvents();
}

void DreamFactoryEngine::drawString(const Common::String &text, int32 packedPoint, int color, int size) {
...
int DreamFactoryEngine::stringWidth(const Common::String &text, int fontId, int size) {
...
void DreamFactoryEngine::fadePalette(const Common::String &target, int steps, bool toBlack) {
	Palette to = {};
	if (!resolveClut(target, to)) {
		...
		debug(1, "DreamFactory: %s('%s', %d) skipped: clut unavailable",
	...
	debug(1, "DreamFactory: %s('%s', %d)", toBlack ? "screentoblack" : "blacktoscreen",
			target.c_str(), steps);
	fadePaletteSteps(from, to, steps);
}

bool DreamFactoryEngine::paletteIsBlack() const {
...
void DreamFactoryEngine::mixClut(const Common::String &nameA, const Common::String &nameB,
		int first, int last, int weight) {
	Palette a = {};
	Palette b = {};
	if (!resolveClut(nameA, a) || !resolveClut(nameB, b)) {
		warning("DreamFactory: mixclut('%s', '%s'): clut not found", nameA.c_str(), nameB.c_str());
		return;
	}
	...
	debug(1, "DreamFactory: mixclut('%s', '%s', %d, %d, %d)",
...
void DreamFactoryEngine::fadePaletteSteps(const Palette &from, const Palette &to, int steps) {
	if (steps < 1)
		steps = 1;
...
bool DreamFactoryEngine::captureScreen(Graphics::ManagedSurface &out) {
	Graphics::Surface *screen = _system->lockScreen();
	if (!screen)
		return false;
	out.copyFrom(*screen);
...
void DreamFactoryEngine::blitScreenBand(const Graphics::Surface &image, const Common::Rect &band) {
	Graphics::Surface *screen = _system->lockScreen();
	if (!screen)
		return;
...
// edge and count up.
void DreamFactoryEngine::runWipe(const Graphics::Surface &incoming, uint16 effect, int steps) {
	if (steps < 1)
		steps = 1;
...
void DreamFactoryEngine::setVisualEffect(uint16 effect, int duration) {
	if (duration < 1)
		duration = 1;
	...
	debug(1, "DreamFactory: visualeffect(%s, %d)%s", effectName ? effectName : "?", duration,
