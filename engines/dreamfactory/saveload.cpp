...
static bool parsePathChunk(Common::SeekableReadStream &in, int64 end,
	...
	for (Common::String &pathSlot : pathSlots)
		if (!readSaveString(in, end, pathSlot))
			return false;
	return !in.err();
}
...
static bool parsePaletteChunk(Common::SeekableReadStream &in, int64 end,
	...
	if (in.read(screenClut.data(), screenClut.byteSize()) != screenClut.byteSize())
		return false;
	for (double &gamma : paletteGamma)
		gamma = in.readDoubleLE();
	return !in.err();
}
...
static bool restoreShopState(Common::Array<Common::SharedPtr<Shop> > &shops, const Common::Array<ShopState> &shopStates) {
	for (const ShopState &shopState : shopStates) {
		Common::SharedPtr<Shop> shop(new Shop());
		if (!shop->open(shopState.name)) {
			warning("DreamFactory: load could not reopen shop '%s'", shopState.name.c_str());
			return false;
		}
		for (const PropState &state : shopState.props) {
			Shop::Prop *prop = shop->findProp(state.name);
			if (!prop) {
				warning("DreamFactory: load shop '%s' missing prop '%s'",
						shopState.name.c_str(), state.name.c_str());
				return false;
			}
...
static bool restoreCastState(Common::Array<Common::SharedPtr<Cast> > &casts, const Common::Array<CastState> &castStates) {
	for (const CastState &castState : castStates) {
		Common::SharedPtr<Cast> cast(new Cast());
		if (!cast->open(castState.name)) {
			warning("DreamFactory: load could not reopen cast '%s'", castState.name.c_str());
			return false;
		}
		for (const ActorState &state : castState.actors) {
			...
			if (!actor) {
				warning("DreamFactory: load cast '%s' missing actor '%s'",
						castState.name.c_str(), state.name.c_str());
				return false;
			}
...
static void restoreTrackState(AudioRuntime &audioRuntime, const Common::Array<TrackState> &trackStates) {
	for (const TrackState &trackState : trackStates) {
		Common::SharedPtr<ThemeTrack> track(new ThemeTrack());
		track->sourceName = trackState.sourceName;
		track->name = trackState.name;
		track->fileData = trackState.fileData;
		track->playlist = trackState.playlist;
		track->loopIdx = trackState.loopIdx;
		track->volume = trackState.volume;
		for (const CueState &cueState : trackState.cues) {
			ThemeTrack::Cue cue;
			cue.name = cueState.name;
			cue.resId = cueState.resId;
			cue.flags = cueState.flags;
			cue.dataOffset = cueState.dataOffset;
			cue.length = cueState.length;
			track->cues.push_back(cue);
		}
		for (const CueState &cueState : trackState.sfxCues) {
			ThemeTrack::Cue cue;
			cue.name = cueState.name;
			cue.resId = cueState.resId;
			cue.flags = cueState.flags;
...
static void restoreCueVolumes(AudioRuntime &audioRuntime, const Common::Array<CueVolumeState> &cueVolumeStates) {
	for (const CueVolumeState &cueVolumeState : cueVolumeStates) {
		for (const Common::SharedPtr<ThemeTrack> &track : audioRuntime.tracks()) {
			if (!cueVolumeState.trackName.empty() &&
					!track->name.equalsIgnoreCase(cueVolumeState.trackName) &&
					!track->sourceName.equalsIgnoreCase(cueVolumeState.trackName))
				continue;
			for (ThemeTrack::Cue &cue : track->sfxCues) {
				if (cue.name.equalsIgnoreCase(cueVolumeState.cueName))
					cue.volume = CLIP(cueVolumeState.volume, 0, 255);
			}
		}
	}
}
...
static void restoreLoopState(LoopRuntime &loopRuntime, bool loopsPaused,
	...
	for (const LoopRuntime::ScheduledLoop &loop : loopStates)
		loopRuntime.restoreLoop(loop);

	loopRuntime.setCricketsPaused(cricketsPaused);
	for (const LoopRuntime::CricketState &cricket : cricketStates)
		loopRuntime.restoreCricket(cricket);
}
...
static void writeCueArray(Common::WriteStream &out, const Common::Array<ThemeTrack::Cue> &cues) {
	...
	for (const ThemeTrack::Cue &cue : cues) {
		writeSaveString(out, cue.name);
		out.writeUint32LE(cue.resId);
		out.writeByte(cue.flags);
		out.writeUint32LE(cue.dataOffset);
		out.writeUint32LE(cue.length);
	}
}
...
static void writeShopChunk(Common::WriteStream &out,
	...
	for (const Common::SharedPtr<Shop> &shopPtr : shops) {
		const Shop &shop = *shopPtr;
		...
		for (uint p = 0; p < shop.propCount(); ++p) {
			...
			for (const Shop::Shape &shape : prop.shapes) {
				payload.writeUint32LE(shape.resId);
				writeSaveString(payload, shape.name);
			}
		}
	}
...
static void writeCastChunk(Common::WriteStream &out,
	...
	for (const Common::SharedPtr<Cast> &cast : casts) {
		writeSaveString(payload, cast->name());
		payload.writeUint32LE(cast->actorCount());
		for (uint a = 0; a < cast->actorCount(); ++a) {
			const Cast::Actor &actor = cast->actor(a);
...
static void writeTrackChunk(Common::WriteStream &out,
	...
	for (const Common::SharedPtr<ThemeTrack> &trackPtr : tracks) {
		const ThemeTrack &track = *trackPtr;
		...
		for (const uint16 cueIndex : track.playlist)
			payload.writeUint16LE(cueIndex);
...
static void writeCueVolumeChunk(Common::WriteStream &out,
	...
	Common::MemoryWriteStreamDynamic payload(DisposeAfterUse::YES);
	uint32 count = 0;
	for (const Common::SharedPtr<ThemeTrack> &track : tracks)
		count += track->sfxCues.size();
	payload.writeUint32LE(count);
	for (const Common::SharedPtr<ThemeTrack> &trackPtr : tracks) {
		const ThemeTrack &track = *trackPtr;
		for (const ThemeTrack::Cue &cue : track.sfxCues) {
			writeSaveString(payload, track.name);
			writeSaveString(payload, cue.name);
			payload.writeSint32LE(cue.volume);
		}
	}
...
static void writeAudioChunk(Common::WriteStream &out, const AudioRuntime &audioRuntime,
	...
	for (const AudioRuntime::ThemeCueSpan &span : audioRuntime.themeSpans()) {
		payload.writeUint32LE(span.startSample);
		writeSaveString(payload, span.name);
	}
...
static void writeVarsChunk(Common::WriteStream &out, const Common::HashMap<Common::String, Value> &vars) {
	...
	for (const Common::HashMap<Common::String, Value>::Node &variable : vars)
		keys.push_back(variable._key);
	Common::sort(keys.begin(), keys.end());
	payload.writeUint32LE(static_cast<uint32>(keys.size()));
	for (const Common::String &key : keys) {
		Common::HashMap<Common::String, Value>::const_iterator value = vars.find(key);
		writeSaveString(payload, key);
...
static void writeLoopChunk(Common::WriteStream &out, const LoopRuntime &loopRuntime) {
	...
	for (const LoopRuntime::ScheduledLoop &loop : loops) {
...
static void writeCricketChunk(Common::WriteStream &out, const LoopRuntime &loopRuntime) {
	...
	for (const LoopRuntime::CricketState &cricket : crickets) {
		writeSaveString(payload, cricket.name);
		payload.writeByte(cricket.paused ? 1 : 0);
	}
...
bool DreamFactoryEngine::canSaveGameStateCurrently(Common::U32String *msg) {
	if (isTourMode(_vm)) {
		if (msg)
...
bool DreamFactoryEngine::canSaveAutosaveCurrently() {
	return !isTourMode(_vm) && canSaveGameStateCurrently();
}

bool DreamFactoryEngine::canLoadGameStateCurrently(Common::U32String *msg) {
...
void DreamFactoryEngine::saveGame(const Common::String &signature) {
...
void DreamFactoryEngine::openGame(const Common::String &signature) {
...
bool DreamFactoryEngine::processPendingLoad() {
...
Common::Error DreamFactoryEngine::loadGameState(int slot) {
	...
	char magic[5] = {};
	if (saveFile->read(magic, 4) != 4 || memcmp(magic, kDreamFactorySaveMagic, 4))
		return Common::Error(Common::kReadingFailed, "Not a DreamFactory save");
	uint32 version = saveFile->readUint32LE();
	if (version != kDreamFactorySaveVersion)
		return Common::Error(Common::kReadingFailed, "Unsupported DreamFactory save version");
	...
	while (!saveFile->eos() && !saveFile->err()) {
		...
		if (!readChunkHeader(*saveFile, tag, end))
			return Common::Error(Common::kReadingFailed, "Corrupt DreamFactory save chunk");
		...
		if (!ok || saveFile->pos() > end)
			return Common::Error(Common::kReadingFailed, Common::String::format("Corrupt DreamFactory save chunk %.4s", tag));
		saveFile->seek(end);
		if (sawEnd)
			break;
	}
	...
	if (!header.seen || !sawEnd)
		return Common::Error(Common::kReadingFailed, "Incomplete DreamFactory save");
	...
	// Resolve every external resource before replacing the running session.
	// Saved path slots must be active while opening those resources; restore
	// the previous registrations if preparation fails.
	Common::String previousPaths[PathRuntime::kPathSlotCount];
	for (uint i = 0; i < ARRAYSIZE(previousPaths); ++i) {
		previousPaths[i] = _pathRuntime.pathSlotValue(i);
		_pathRuntime.setPathSlot(i, pathSlots[i]);
	}
	Common::Array<Common::SharedPtr<Shop> > restoredShops;
	Common::Array<Common::SharedPtr<Cast> > restoredCasts;
	Common::SharedPtr<Stage> restoredStage;
	Common::ScopedPtr<Set> restoredSet;
	bool prepared = restoreShopState(restoredShops, shopStates) &&
			restoreCastState(restoredCasts, castStates);
	if (prepared && !header.stageName.empty()) {
		restoredStage.reset(new Stage());
		prepared = restoredStage->open(header.stageName);
	}
	if (prepared && !header.setFileName.empty()) {
		restoredSet.reset(new Set());
		prepared = restoredSet->open(header.setFileName);
	}
	if (!prepared) {
		for (uint i = 0; i < ARRAYSIZE(previousPaths); ++i)
			_pathRuntime.setPathSlot(i, previousPaths[i]);
		return Common::Error(Common::kReadingFailed, "Could not restore saved game resources");
	}
	...
	if (varsSeen) {
		...
		for (const Common::HashMap<Common::String, Value>::Node &variable : vars)
			_vm.globalVars()[variable._key] = variable._value;
	}

	_propRuntime.shops().swap(restoredShops);
	_actorRuntime.casts().swap(restoredCasts);
	...
	stageRuntime().restoreSnapshot(stageSnapshot, restoredStage);
	...
	setRuntime().restoreSnapshot(setSnapshot, restoredSet);
...
Common::Error DreamFactoryEngine::saveGameState(int slot, const Common::String &desc, bool isAutosave) {
	...
	saveFile->write(kDreamFactorySaveMagic, 4);
	saveFile->writeUint32LE(kDreamFactorySaveVersion);
