...
DreamFactoryEngine::DreamFactoryEngine(OSystem *syst, const DreamFactoryGameDescription &gameDesc) :
		Engine(syst), _gameDescription(gameDesc),
		_gameSupport(createGameSupport(gameDesc.gameType)),
		_rnd("dreamfactory"), _pathRuntime(*_gameSupport) {
	_cursorRuntime.setExecutableName(_gameSupport->profile().runtimeExecutable);
}

DreamFactoryEngine::~DreamFactoryEngine() {
}

int DreamFactoryEngine::getGameType() const {
	return _gameDescription.gameType;
}

const char *DreamFactoryEngine::getGameId() const {
	return _gameDescription.desc.gameId;
}

Common::Language DreamFactoryEngine::getLanguage() const {
	return _gameDescription.desc.language;
}

Common::Platform DreamFactoryEngine::getPlatform() const {
	return _gameDescription.desc.platform;
}

bool DreamFactoryEngine::hasFeature(EngineFeature f) const {
...
void DreamFactoryEngine::sendToBoot(const Common::String &message, const Common::Array<Value> &args) {
	if (!_bootScript) {
		warning("DreamFactory: sendtoboot('%s') before BOOTFILE loaded", message.c_str());
		return;
	}
...
Value DreamFactoryEngine::sendToBootFx(const Common::String &message, const Common::Array<Value> &args) {
	if (!_bootScript) {
		warning("DreamFactory: sendtobootfx('%s') before BOOTFILE loaded", message.c_str());
		return Value();
	}
...
Common::String DreamFactoryEngine::hitTest(int32 packedPoint) {
...
bool DreamFactoryEngine::pointInButton(const Common::String &flat,
		const Common::String &button, int32 packedPoint) {
	return _stageRuntime.pointInButton(flat, button, packedPoint);
}

bool DreamFactoryEngine::pointInPainting(const Common::String &scene,
	...
	debug(1, "DreamFactory: pointinpainting('%s', '%s', '%s', %d,%d) -> %s",
...
int DreamFactoryEngine::tick() {
	return static_cast<int>((static_cast<uint64>(_system->getMillis()) * 60 / 1000));
}

int DreamFactoryEngine::frameCounter() {
	return _frameCounter;
}

int DreamFactoryEngine::calcDeg(int32 a, int32 b) {
...
int DreamFactoryEngine::calcVectX(int deg, int dist) {
...
int DreamFactoryEngine::calcVectY(int deg, int dist) {
...
int DreamFactoryEngine::calcDist(int32 a, int32 b) {
	const int16 ax = static_cast<int16>(a >> 16);
	const int16 ay = static_cast<int16>(a & 0xffff);
	const int16 bx = static_cast<int16>(b >> 16);
	const int16 by = static_cast<int16>(b & 0xffff);
...
int DreamFactoryEngine::calcMod(int a, int b) {
	return b != 0 ? a % b : 0;
}

// cursor(...) -> TI.EXE FUN_00446920, with the script name already resolved
// to a PE resource name by the VM (see DreamFactoryEngine::setCursorResource).
void DreamFactoryEngine::setCursorResource(const Common::String &resourceName) {
	if (setGameCursor(resourceName)) {
		...
		warning("DreamFactory: cursor resource '%s' not found in the game executable",
				resourceName.c_str());
	} else {
		...
		warning("DreamFactory: cursor '%s' unavailable: no game executable with cursor "
				"resources was found", resourceName.c_str());
	}
}

bool DreamFactoryEngine::actionFrame(int n) {
	if (n < 1 || n > 2)
		return false;
	return (_actionFrameMask & (1 << (n - 1))) != 0;
}

int DreamFactoryEngine::randomNumber(int n) {
	if (n < 1)
		return 0;
...
int DreamFactoryEngine::getFrameRate() {
	return _framePacingRuntime.getFrameRate();
}

int DreamFactoryEngine::setFrameRate(int newRate) {
	return _framePacingRuntime.setFrameRate(newRate);
}

bool DreamFactoryEngine::keyAborts(const Common::String *resource, const Common::String *key,
		const bool *enabled) {
	if (enabled)
		_keyAborts = *enabled;
	return _keyAborts;
...
bool DreamFactoryEngine::optionKey() {
...
bool DreamFactoryEngine::shiftKey() {
	return optionKey();
}

// Deferred input queue:
// ScummVM has several nested DreamFactory loops that pump backend events for a
...
void DreamFactoryEngine::noteDeferredInputEvent(const Common::Event &event) {
	if (event.type != Common::EVENT_LBUTTONDOWN && event.type != Common::EVENT_LBUTTONUP)
		return;
	debug(1, "DreamFactory: queued %s at (%d,%d) arrived t=%u",
			event.type == Common::EVENT_LBUTTONDOWN ? "LBUTTONDOWN" : "LBUTTONUP",
			event.mouse.x, event.mouse.y, _system->getMillis());
}

bool DreamFactoryEngine::pollScriptEvent(Common::Event &event) {
...
bool DreamFactoryEngine::pollInputStateEvents() {
...
bool DreamFactoryEngine::pumpCursorMotionEvents() {
	const Common::Point oldMouse = _eventMan->getMousePos();
	Common::Event event;
...
void DreamFactoryEngine::presentCursorIfDirty() {
...
void DreamFactoryEngine::reassertCursorVisibility() {
	// SDL may reveal the host cursor after focus/active-area transitions. Room
	// gameplay presents cursor motion without changing the DreamFactory cursor
	// resource, so explicitly reapply ScummVM's current software-cursor state.
	const bool visible = CursorMan.isVisible();
	CursorMan.showMouse(visible);
}

bool DreamFactoryEngine::delayMillisWithCursorUpdates(uint32 delayMillis) {
...
void DreamFactoryEngine::delayTicks(int ticks) {
	if (ticks <= 0)
		return;
...
void DreamFactoryEngine::makeLoop(const Common::String &kind, const Common::String &target,
		const Common::String &message, int delay) {
	_loopRuntime.makeLoop(kind, target, message, delay);
}

void DreamFactoryEngine::stopLoop(const Common::String &kind, const Common::String &target) {
	_loopRuntime.stopLoop(kind, target);
}

void DreamFactoryEngine::pauseLoop(const Common::String &kind, bool paused) {
	_loopRuntime.pauseLoop(kind, paused);
}

void DreamFactoryEngine::makeCricket(const Common::String &name) {
	_loopRuntime.makeCricket(*this, name);
}

void DreamFactoryEngine::stopCricket(const Common::String &name) {
	_loopRuntime.stopCricket(name);
}

void DreamFactoryEngine::pauseCricket(const Common::String &kind, bool paused) {
	_loopRuntime.pauseCricket(kind, paused);
}

void DreamFactoryEngine::processScheduledLoops() {
	_loopRuntime.processScheduledLoops(*this);
}

void DreamFactoryEngine::forceUpdate() {
	...
	if (_puppetRuntime.isVisible()) {
		...
		_setRuntime.acknowledgeScreenUpdate();
	...
	debug(2, "DreamFactory: forceupdate()");
}

Common::Error DreamFactoryEngine::run() {
	// The original is a 512x384 8-bit palettised WinG title (see the
	// kScreenWidth/kScreenHeight comment in dreamfactory.h).
	...
	// resources with info tag 0x0FA1. Resource 1 is the boot script: its first
	// definition, `boot()`, runs engine setup, the CD check, the intro movies,
	// and the menu branch; later definitions are event handlers such as
	// `keydown()`. Resource 2 is the GLOBAL function library (`changeset`,
	// `initall`, `advanceday`, `advancetour`, ...).
	//
	// Both scripts stay registered on the VM's dispatch scope chain for the
	// lifetime of the session, mirroring the TI.EXE chain [current script,
	// global library] built by FUN_0040ad80.
	Common::File bootFile;
	if (!bootFile.open("BOOTFILE")) {
		warning("DreamFactory: could not locate DATA/BOOTFILE");
		return Common::kNoGameDataFoundError;
	}

	Archive boot;
	if (!boot.open(bootFile.readStream(bootFile.size()), "BOOTFILE")) {
		warning("DreamFactory: BOOTFILE present but failed LPPALPPA validation");
		return Common::kUnknownError;
	}

	for (const Archive::Resource &res : boot.resources()) {
		if (res.empty || res.info != Script::kScriptInfoTag)
			continue;
		Common::ScopedPtr<Common::SeekableReadStream> scriptStream(
				boot.createReadStreamForResource(res));
		Common::ScopedPtr<Script> script(new Script());
		bool parsed = scriptStream && script->parse(scriptStream.get());
		if (!parsed) {
			warning("DreamFactory: failed to parse BOOTFILE script resource %u", res.id);
			continue;
		}
		if (!_bootScript)
	...
	if (!_bootScript) {
		warning("DreamFactory: BOOTFILE has no script resource");
		return Common::kUnknownError;
	}

	if (!_gameSupport->patchBootScript(*_bootScript))
		warning("DreamFactory: boot script CD check not found; running unmodified");
	...
	while (!shouldQuit()) {
		...
		while (pollScriptEvent(event)) {
			...
			if (event.type == Common::EVENT_MOUSEMOVE) {
				...
				debug(1, "DreamFactory: LBUTTONUP at (%d,%d) dispatch t=%u (%s)",
						event.mouse.x, event.mouse.y, _system->getMillis(),
						_lastScriptEventWasDeferred ? "queued" : "fresh");
			} else if (event.type == Common::EVENT_LBUTTONDOWN) {
				debug(1, "DreamFactory: LBUTTONDOWN at (%d,%d) dispatch t=%u (%s)",
						event.mouse.x, event.mouse.y, _system->getMillis(),
						_lastScriptEventWasDeferred ? "queued" : "fresh");
				...
				if (!handled)
					warning("DreamFactory: boot script has no mousedown handler");
				...
				if (!key.empty()) {
					...
					scriptEventHandled = true;
					if (!handled)
						warning("DreamFactory: boot script has no %s handler",
