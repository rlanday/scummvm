...
#ifndef DREAMFACTORY_SCRIPT_H
#define DREAMFACTORY_SCRIPT_H
...
/**
 * Parser for DreamFactory script resources, found in BOOTFILE and in the .STG /
...
public:
	...
	enum MethodOpcode {
		...
		kMethodActorExists      = 0x4E37,
		kMethodPropExists       = 0x4E38,
		...
		kMethodCountFlats       = 0x4E43,
		kMethodIndexToFlat      = 0x4E44,
		kMethodFlatToIndex      = 0x4E45,
		...
		kMethodShiftKey         = 0x4E5B,
		kMethodCalcVectX        = 0x4E5D,
		kMethodCalcVectY        = 0x4E5E,
		kMethodCameraXYZ        = 0x4E5F,
		kMethodPlayerXYZ        = 0x4E60,
	...
	/**
	 * Index of the nearest enclosing kOpIf before @p index, or -1 if none
	 * is found or the index is invalid. Scans backward, passing over IF/ENDIF
	 * pairs for blocks that already ended before the supplied instruction.
	 */
	int findEnclosingIf(uint32 index) const;
	...
	/**
	 * Name of the DreamFactory builtin method @p opcode, or nullptr if unknown.
	 *
