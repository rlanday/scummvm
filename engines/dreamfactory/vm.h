...
#ifndef DREAMFACTORY_VM_H
#define DREAMFACTORY_VM_H
...
/**
 * Tagged value held on the DreamFactory VM operand stack.
 *
 * The precise runtime value model of the DreamFactory VM is still being recovered
...
/**
 * Executes a parsed DreamFactory Script. This is the structural harness that
...
public:
	 ...
	 */
	void setHost(DreamFactoryEngine *host) { _host = host; }
	...
	/**
	 * One handler-lookup entry: a borrowed script and optional receiver names.
	 * When either name is nonempty, callFunction() installs both as the context
	 * for the matching handler, then restores the caller's context. Two empty
	 * names instead inherit the active context; they do not clear it.
	 * These names are independent of the handler's parameters/local variables.
	 */
	struct LibraryScope {
	 ...
	 * The supplied array is in VM storage order (last entry searched first),
	 * unlike DreamFactoryEngine's dispatch helpers, which accept lookup order.
	 * Swaps array storage, not script ownership; scripts must remain alive for
	 * every active or saved chain that can still use them.
	 */
...
private:
	...
	DreamFactoryEngine *_host; ///< Engine host for effectful builtins; null = no-op. Not owned.
