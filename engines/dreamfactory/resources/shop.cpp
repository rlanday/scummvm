...
bool Shop::open(const Common::String &name) {
	...
	if (!_resourceFile.open(name, "shop"))
		return false;
	...
	if (_master < 0) {
		warning("DreamFactory: shop '%s' has no master header", name.c_str());
		_resourceFile.close();
		return false;
	}
	...
	if (!hdr) {
		warning("DreamFactory: shop '%s' master header truncated", name.c_str());
		_master = -1;
		_resourceFile.close();
		return false;
	}
	...
	if (scriptRes != 0) {
		...
		if (scriptIdx >= 0) {
			...
			if (!_script)
				warning("DreamFactory: shop '%s' script res %u failed to parse", name.c_str(), scriptRes);
		} else {
			warning("DreamFactory: shop '%s' script res %u missing", name.c_str(), scriptRes);
		}
	}
	...
	for (const ResourceView propRecord : propRecords) {
		const byte *entry = propRecord.dataAt(0, kMasterPropStride);
		...
		if (!pm) {
			warning("DreamFactory: shop '%s' prop master %u missing", name.c_str(), masterId);
			continue;
		}
	...
	debugC(1, kDebugResources, "DreamFactory: opened shop '%s': %u prop(s)", name.c_str(), _props.size());
	return true;
}
...
Common::SharedPtr<CelImage> Shop::celResource(uint32 resId) const {
	...
	const Archive::Resource &res = _resourceFile.archive().getResource(static_cast<uint32>(idx));
	uint16 width = static_cast<uint16>(res.info >> 16);
	uint16 height = static_cast<uint16>(res.info & 0xffff);
	if (!width || !height)
		return Common::SharedPtr<CelImage>();

	Common::ScopedPtr<Common::SeekableReadStream> s(_resourceFile.archive().createReadStreamForResource(res));
	if (!s)
		return Common::SharedPtr<CelImage>();
...
Shop::PropCellResult Shop::resolvePropCel(const Prop &prop, int angle) const {
	...
	if (!shape) {
		debugC(1, kDebugResources, "DreamFactory: renderProp('%s'): shape '%s' not in master",
	...
	const byte *sh = shapeView.dataAt(0, kShapeCellTableOffset);
	if (!sh) {
		debugC(1, kDebugResources, "DreamFactory: renderProp('%s'): shape res %u missing",
	...
	if (!bestData) {
		debugC(2, kDebugResources, "DreamFactory: renderProp('%s'): no cell for pose %u in shape '%s'",
	...
	if (!result.cel) {
		debugC(1, kDebugResources, "DreamFactory: renderProp('%s'): cel res %u decode failed",
