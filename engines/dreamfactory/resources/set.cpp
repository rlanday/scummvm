...
bool Set::applyFrameResource(uint32 frameId, FrameSequence &seq) const {
	...
	if (idx < 0) {
		warning("DreamFactory: set '%s' references missing frame res %u", _name.c_str(), frameId);
		return false;
	}
	...
	if (seq.applyFrame(frame.dataAt(0, frame.size()), static_cast<uint32>(frame.size())) == 0) {
		warning("DreamFactory: set '%s' frame %u decode failed", _name.c_str(), frameId);
		return false;
	}
	return true;
}
...
bool Set::open(const Common::String &name) {
	...
	if (!_resourceFile.open(name, "set"))
		return false;

	_master = findMasterHeaderIndex(_resourceFile.archive());
	if (_master < 0) {
		warning("DreamFactory: set '%s' has no master header", name.c_str());
		reset();
		return false;
	}
	...
	if (!hdr) {
		warning("DreamFactory: set '%s' master header truncated", name.c_str());
		reset();
		return false;
	}
	...
	if (_sceneTable < 0) {
		warning("DreamFactory: set '%s' references missing scene table %u", name.c_str(), sceneTableId);
		reset();
		return false;
	}
	// The scene table is a tight array of fixed-size records, no count header.
	_sceneCount = _resourceFile.archive().getResource(static_cast<uint32>(_sceneTable)).length / kSceneRecordStride;

	_scripts.resize(_resourceFile.archive().getResourceCount());
	for (uint32 i = 0; i < _resourceFile.archive().getResourceCount(); ++i) {
		const Archive::Resource &res = _resourceFile.archive().getResource(i);
		if (res.empty || res.info != Script::kScriptInfoTag)
			continue;
		Common::ScopedPtr<Common::SeekableReadStream> stream(_resourceFile.archive().createReadStreamForResource(res));
		Common::SharedPtr<Script> script(new Script());
		if (stream && script->parse(stream.get()))
			_scripts[i] = script;
		else
			warning("DreamFactory: failed to parse set '%s' script resource %u", name.c_str(), res.id);
	}

	debugC(1, kDebugResources, "DreamFactory: opened set '%s': %ux%u, %u scene(s)",
...
bool Set::renderScene(uint32 scene, uint32 table, uint32 angle, FrameSequence &seq) {
	const RecordRange panorama = panoramaTable(scene, table);
	if (!panorama.valid()) {
		warning("DreamFactory: set '%s' scene %u panorama table %u missing", _name.c_str(), scene, table);
		return false;
	}
	if (angle >= panorama.size()) {
		warning("DreamFactory: set '%s' scene %u table %u angle %u out of range (%u)",
