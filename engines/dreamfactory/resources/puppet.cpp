...
Common::SharedPtr<Script> Puppet::parseScriptResource(uint32 resId) const {
	...
	Common::ScopedPtr<Common::SeekableReadStream> s(_resourceFile.archive().createReadStreamForResource(static_cast<uint32>(idx)));
...
bool Puppet::open(const Common::String &name) {
	...
	if (!_resourceFile.open(name, "puppet"))
		return false;

	_master = findMasterHeaderIndex(_resourceFile.archive());
	if (_master < 0) {
		warning("DreamFactory: puppet '%s' has no master header", name.c_str());
		_resourceFile.close();
		return false;
	}
	...
	if (!hdr) {
		warning("DreamFactory: puppet '%s' master header truncated", name.c_str());
		_master = -1;
		_resourceFile.close();
		return false;
	}
	...
	if (!tableData) {
		warning("DreamFactory: puppet '%s' script table missing", name.c_str());
		return true;
	}
	...
	for (const ResourceView scriptRecord : scriptRecords) {
	...
	debugC(1, kDebugResources, "DreamFactory: opened puppet '%s': name '%s', %u script(s), %u action(s)",
...
Common::SharedPtr<CelImage> Puppet::celResource(uint32 resId) const {
	...
	if (!ok) {
		warning("DreamFactory: puppet '%s' could not decode cel resource %u",
...
bool Puppet::decodeActionAudio(const ActionEntry &action, Common::Array<byte> &pcm) const {
	...
	if (idx < 0)
		return false;
	const Archive::Resource &res = _resourceFile.archive().getResource(static_cast<uint32>(idx));
	const ResourceView audio = resourcePayloadView(_resourceFile.data(), res);
