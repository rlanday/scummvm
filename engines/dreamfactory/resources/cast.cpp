...
bool Cast::open(const Common::String &name) {
	...
	if (!_resourceFile.open(name, "cast"))
		return false;

	_master = findMasterHeaderIndex(_resourceFile.archive());
	if (_master < 0) {
		warning("DreamFactory: cast '%s' has no master header", name.c_str());
		_resourceFile.close();
		return false;
	}
	...
	if (!hdr) {
		warning("DreamFactory: cast '%s' master header truncated", name.c_str());
		_master = -1;
		_resourceFile.close();
		return false;
	}
	...
	if (scriptRes != 0) {
		...
		if (scriptIdx >= 0) {
			Common::ScopedPtr<Common::SeekableReadStream> s(_resourceFile.archive().createReadStreamForResource(static_cast<uint32>(scriptIdx)));
			Common::ScopedPtr<Script> script(new Script());
			if (s && script->parse(s.get()))
				_script.reset(script.release());
			if (!_script)
				warning("DreamFactory: cast '%s' script res %u failed to parse", name.c_str(), scriptRes);
		} else {
			warning("DreamFactory: cast '%s' script res %u missing", name.c_str(), scriptRes);
		}
	}
	...
	for (const ResourceView actorRecord : actorRecords) {
		const byte *entry = actorRecord.dataAt(0, kMasterActorStride);
		...
		if (!am) {
			warning("DreamFactory: cast '%s' actor master %u missing", name.c_str(), masterId);
			continue;
		}
		...
		for (const ResourceView shapeRecord : shapeRecords) {
		...
		if (sIdx >= 0) {
			Common::ScopedPtr<Common::SeekableReadStream> s(_resourceFile.archive().createReadStreamForResource(static_cast<uint32>(sIdx)));
	...
	debugC(1, kDebugResources, "DreamFactory: opened cast '%s': %u actor(s)", name.c_str(), _actors.size());
	return true;
}
...
Cast::ActorCellResult Cast::resolveActorCell(const Actor &actor, int angle) const {
	...
	if (!shape) {
		debugC(1, kDebugResources, "DreamFactory: renderActor('%s'): shape '%s' not in master",
	...
	if (!sh) {
		debugC(1, kDebugResources, "DreamFactory: renderActor('%s'): shape res %u missing",
	...
	if (!bestData) {
		debugC(1, kDebugResources, "DreamFactory: renderActor('%s'): no cell for pose %u in shape '%s'",
...
Cast::ActorRenderResult Cast::renderWorldActor(const Actor &actor, const Shop::WorldCamera &camera,
	...
	if (fIdx < 0)
		return result;
	const Archive::Resource &fres = _resourceFile.archive().getResource(static_cast<uint32>(fIdx));
	uint16 w = static_cast<uint16>(fres.info >> 16);
	uint16 h = static_cast<uint16>(fres.info & 0xffff);
	Common::ScopedPtr<Common::SeekableReadStream> fs(_resourceFile.archive().createReadStreamForResource(fres));
	if (!fs)
		return result;
	bool ok = decodeCel(*fs, w, h, result.cel);
	if (!ok) {
		debugC(1, kDebugResources, "DreamFactory: renderActor('%s'): cel res %u decode failed",
