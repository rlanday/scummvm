...
Common::String Script::getPoolString(uint32 offset) const {
	...
	for (uint32 i = 0; i < len; ++i) {
		...
		// Printable ASCII runs from space (32) through tilde (126), inclusive.
		if (c < 32 || c > 126)
			return Common::String();
...
const Common::String &Script::getSelfRelStringRef(uint32 index) const {
	if (index >= _code.size() || index >= _selfRelStringCache.size())
		return _emptyString;
	// Cache decoded pool strings so hot VM paths can use them by reference
	// without copying a Common::String for each atom. neutralizeRange() clears
	// the cache entries for modified instructions; callers must not rely on
	// those entries retaining their values across such edits.
...
void Script::neutralizeRange(uint32 first, uint32 last) {
	...
	if (last >= _code.size())
		last = _code.size() - 1;
	// At statement boundaries, the VM skips kOpPushInt as separator/padding
	// without evaluating it or pushing a value (native TI.EXE 0x0040c1a1-0x0040c1b2).
	// Replacing a complete statement/block, such as the boot CD-check IF through
	// its ENDIF, therefore leaves a run of no-ops. Keep the records in place so
	// later instruction indices and self-relative string offsets stay unchanged.
	// This is not a general expression-editing operation: inside an expression,
	// kOpPushInt still evaluates to its integer operand, here zero.
...
// by hand; this is the verbatim DreamFactory script builtin vocabulary.
...
int Script::findEnclosingIf(uint32 index) const {
	if (index >= _code.size())
		return -1;

	int depth = 0;
	while (index > 0) {
		const uint16 opcode = _code[--index].opcode;
		// An ENDIF belongs to a block that ended before our starting point.
		// Pass over its matching IF rather than treating it as the enclosing IF.
		if (opcode == kOpEndIf) {
			++depth;
		} else if (opcode == kOpIf) {
			if (depth == 0)
