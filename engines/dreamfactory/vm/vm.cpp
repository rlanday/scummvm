...
#include "dreamfactory/vm/arithmetic.h"
...
Value ScriptVM::getVar(const Common::String &name, const Common::String &key) const {
	if (!_locals.empty() && _locals.back().contains(key)) {
		...
		if (gDebugLevel > 0 && key == "tour")
			debugC(1, kDebugVM, "DreamFactory: var tour local -> %s", v.toString().c_str());
		return v;
	}
	if (_vars.contains(key)) {
		Value v = _vars[key];
		if (gDebugLevel > 0 && key == "tour")
			debugC(1, kDebugVM, "DreamFactory: var tour global -> %s", v.toString().c_str());
		return v;
	}
	if (gDebugLevel > 0 && key == "tour")
		debugC(1, kDebugVM, "DreamFactory: var tour unbound");
...
void ScriptVM::setVar(const Common::String &name, const Value &v) {
	...
	if (!_locals.empty() && _locals.back().contains(key)) {
		if (logGameVariable)
			debugC(1, kDebugVM, "DreamFactory: game var %s local %s -> %s",
	...
	if (logGameVariable) {
		...
		debugC(1, kDebugVM, "DreamFactory: game var %s global %s -> %s",
...
void ScriptVM::run(const Script &script, uint32 maxSteps) {
	...
	while (_pc < script.getInstructionCount() && steps < maxSteps) {
		...
		if (_trace) {
			debugC(1, kDebugVM, "  %4u: %-8s a=%#06x b=%#010x  [stack=%u]", here,
...
Value ScriptVM::applyBinary(uint16 opcode, const Value &lhs, const Value &rhs) {
	...
	switch (opcode) {
	case Script::kOpAdd: return Value::makeInt(addWords(a, b));
	case Script::kOpSub: return Value::makeInt(subtractWords(a, b));
	case Script::kOpMul: return Value::makeInt(multiplyWords(a, b));
	case Script::kOpDiv: {
		int32 result = 0;
		if (!divideWords(a, b, result))
			warning("DreamFactory: invalid integer division (%d / %d)", a, b);
...
Value ScriptVM::decodeAtom(const Script &script, uint32 &pc) {
	...
	if (_exprDepth >= 64) {
		debugC(1, kDebugVM, "DreamFactory: expression nesting too deep, abandoning atom");
		return Value();
	}
	_exprDepth++;
...
Value ScriptVM::decodeAtomInner(const Script &script, uint32 &pc) {
	...
	case Script::kOpSub: {
		...
		pc++;
		Value v = decodeAtom(script, pc);
		return Value::makeInt(subtractWords(0, v.intValue));
	}
	...
	default: {
		...
		if (pc < count && script.getInstruction(pc).opcode == Script::kOpOpenParen) {
			...
			// sendtoprop (0x2f17), sendtoshop (0x2f1b), sendtopainting (0x2f22),
			// sendtoset (0x2f23), sendtobutton (0x2f24), sendtoflat (0x2f25),
			// sendtostage (0x2f26), sendtoboot (0x2f31), and the value-returning
			// 0x4e74-0x4e7f fx dispatch family pass their final argument
			// UNevaluated: it is a message `name(args)` matched
...
Value ScriptVM::dispatchMessageBuiltin(const Script &script, uint32 &pc, uint16 opcode) {
	...
	if (_trace)
		debugC(1, kDebugVM, "    message #%#06x -> %s(%u args)", opcode, message->c_str(), msgArgs.size());
...
Value ScriptVM::callFunction(const Common::String &name, const Common::Array<Value> &args,
	...
	if (logGameDispatch) {
		...
		debugC(1, kDebugVM, "DreamFactory: game script %s(%s) ctx self='%s' prop='%s'",
	...
	if (_callDepth >= 64) { // TI.EXE has no explicit guard; protect the engine
		warning("DreamFactory: script call depth overflow at '%s'", name.c_str());
		return result;
	}
	...
	for (int li = static_cast<int>(_libraries.size()) - 1; li >= 0; --li) {
		...
		if (logTransitionDispatch)
			debugC(1, kDebugVM, "DreamFactory: script '%s' matched scope %d/%u body %u (%u args)",
		...
		if (handled)
			*handled = true;
		if (logTransitionDispatch)
			debugC(1, kDebugVM, "DreamFactory: script '%s' handled by scope %d -> %s",
					name.c_str(), li, result.toString().c_str());
		if (logGameDispatch)
			debugC(1, kDebugVM, "DreamFactory: game script %s(%s) -> %s",
					name.c_str(), gameArgText.c_str(), result.toString().c_str());
		if (_trace)
			debugC(1, kDebugVM, "  dispatch %s(%u args) -> %s", name.c_str(), args.size(),
					result.toString().c_str());
		return result;
	}

	if (logTransitionDispatch)
		debugC(1, kDebugVM, "DreamFactory: script '%s' had no matching definition (%u scopes)",
				name.c_str(), _libraries.size());
	if (_trace)
		debugC(1, kDebugVM, "  dispatch %s: no matching definition", name.c_str());
	return result;
}
...
ScriptVM::RunResult ScriptVM::runBody(const Script &script, uint32 pc, Value &result,
	...
	while (pc < count && (maxSteps == 0 || executed < maxSteps)) {
		...
		case Script::kOpForNext: {
			...
			int32 next = addWords(getVar(frame.var).intValue, 1);
		...
		default: {
			...
			if (op == Script::kOpPushSym) {
				...
				if (nextOp == Script::kOpEq) {
					...
					setVar(var, v);
					if (_trace)
						debugC(1, kDebugVM, "    assign %s = %s", var.c_str(), v.toString().c_str());
					pc = p;
					break;
				}
		...
		if (_trace) {
			debugC(1, kDebugVM, "  stmt %4u: %-8s -> pc=%u", here,
	...
	if (pc < count && maxSteps != 0 && executed >= maxSteps)
		warning("DreamFactory: script body stopped after %u statements at pc %u", maxSteps, pc);
	return kRunDone;
}
