...
#ifndef DREAMFACTORY_AUDIO_AUDIO_RUNTIME_H
#define DREAMFACTORY_AUDIO_AUDIO_RUNTIME_H
...
public:
	...
	void applyLiveAudioVolumes(DreamFactoryEngine &engine);
	void prepareThemeSpans(const ThemeTrack &track);
	bool startThemeStream(DreamFactoryEngine &engine, const Common::SharedPtr<ThemeTrack> &track, uint32 startSample);
	bool playSoundCue(DreamFactoryEngine &engine, const Common::String &name, Audio::SoundHandle &handle,
	...
	void playTheme(DreamFactoryEngine &engine, const Common::String &name);
	void haltTheme(DreamFactoryEngine &engine);
	void playSound(DreamFactoryEngine &engine, const Common::String &name, int mode);
	void playVoice(DreamFactoryEngine &engine, const Common::String &name);
	void haltSound(DreamFactoryEngine &engine, int which);
	void haltVoice(DreamFactoryEngine &engine);
	void themeVolume(DreamFactoryEngine &engine, const Common::String &name, int volume);
	int getWaveVolume(DreamFactoryEngine &engine);
	int setWaveVolume(DreamFactoryEngine &engine, int newLevel);
	int getSoundVolume(DreamFactoryEngine &engine, const Common::String &name);
	int setSoundVolume(DreamFactoryEngine &engine, const Common::String &name, int newVolume);
	Common::String currentTheme(DreamFactoryEngine &engine, int which);
	Common::String currentSound(DreamFactoryEngine &engine, int which);
	Common::String currentVoice(DreamFactoryEngine &engine);
	bool voiceDone(DreamFactoryEngine &engine);
